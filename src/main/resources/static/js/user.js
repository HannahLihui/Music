...
var vm = new Vue({
    ...
    methods: {
        ...
        searchBy() {
            this.loadings();
            this.$http.post('/user/findByConPage', this.searchEntity).then(result => {
                console.log(result);
        ...
        search() {
            this.loadings();
            this.$http.post('/user/findAll').then(result => {
                console.log(result);
                this.user= result.body;
        ...
        sureEdit(editor) {
            ...
            //调用更新数据的接口
            this.$http.post('/user/update', JSON.stringify(this.editor)).then(result => {
        ...
        sureDelete(ids) {
            this.$confirm('你确定永久删除此用户信息？', '提示', {
                ...
                this.$http.post('/user/delete', JSON.stringify(ids)).then(result => {
                    if (result.body.success) {
                        //删除成功
                        this.selectIds = []; //清空选项
                        this.$message({
        ...
        save(editor) {
            this.$refs[editor].validate((valid) => {
                if (valid) {
                    ...
                    this.showSave = false;
                    //调用保存的接口
                    this.$http.post('/user/create', JSON.stringify(this.editor)).then(result => {
                        if (result.body.success) {
                            //保存成功
                            this.$message({
                                type: 'success',
        ...
        deleteSelect(rows) {
            if (rows) {
                rows.forEach(row => {
                    ...
                    this.$refs.user.toggleRowSelection(row);
                });
                //调用删除方法
