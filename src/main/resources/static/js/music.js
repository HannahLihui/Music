...
var vm = new Vue({
    ...
    methods: {
        ...
        searchBy() {
            this.loadings();
            this.$http.post('/music/findByConPage', this.searchEntity).then(result => {
                console.log(result);
        ...
        //清空已选择的
        clearSelect() {
            this.$refs.music.clearSelection();
        },
        ...
        sureEdit(editor) {
            ...
            //调用更新数据的接口
            this.$http.post('/music/update', JSON.stringify(this.editor)).then(result => {
        ...
        sureDelete(ids) {
            this.$confirm('你确定永久删除此用户信息？', '提示', {
                ...
                this.$http.post('/music/delete', JSON.stringify(ids)).then(result => {
                    if (result.body.success) {
                        //删除成功
                        this.selectIds = []; //清空选项
                        this.$message({
        ...
        save(editor) {
            this.$refs[editor].validate((valid) => {
                if (valid) {
                    ...
                    this.showSave = false;
                    //调用保存的接口
                    this.$http.post('/music/create', JSON.stringify(this.editor)).then(result => {
                        if (result.body.success) {
                            //保存成功
                            this.$message({
                                type: 'success',
        ...
        handleEdit(id) {
            ...
            this.editor = {}; //清空表单
            //查询当前id对应的数据
            this.$http.post('/music/findById', {id: id}).then(result => {
                this.fileList.forEach(row => {
                    row.url = result.body[0].img; //将图片的URL地址赋值给file-list展示出来
                });
            ...
            if (this.multipleSelection.length === 1) {
                this.multipleSelection.forEach(row => {
                    ...
                    this.fileList.forEach(file => {
                        file.url = row.img; //将图片的URL地址赋值给file-list展示出来
                    });
                    //移除element-ui表单校验残留
                    this.$refs['editor'].resetFields();
        ...
        deleteSelect(rows) {
            if (rows) {
                rows.forEach(row => {
                    ...
                    this.$refs.music.toggleRowSelection(row);
                });
                //调用删除方法
        ...
        handleSuccess(res, file, fileList) {
            ...
            if (file.response.success) {
                this.editor.img = file.response.message; //将返回的文件储存路径赋值image字段
            }
        },
        //删除文件之前的钩子函数
