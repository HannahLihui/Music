...
var vm = new Vue({
    ...
    methods: {
        ...
        sureEdit(editor) {
            ...
            //调用更新数据的接口
            this.$http.post('/news/update', JSON.stringify(this.editor)).then(result => {
        ...
        sureDelete(ids) {
            this.$confirm('你确定永久删除此用户信息？', '提示', {
                ...
                this.$http.post('/news/delete', JSON.stringify(ids)).then(result => {
                    if (result.body.success) {
                        //删除成功
                        this.selectIds = []; //清空选项
                        this.$message({
        ...
        save(editor) {
            this.$refs[editor].validate((valid) => {
                if (valid) {
                    ...
                    this.showSave = false;
                    //调用保存的接口
                    this.$http.post('/news/create', JSON.stringify(this.editor)).then(result => {
                        if (result.body.success) {
                            //保存成功
                            this.$message({
                                type: 'success',
        ...
        handleEdit(id) {
            ...
            this.editor = {}; //清空表单
            //查询当前id对应的数据
            this.$http.post('/news/findById', {id: id}).then(result => {
